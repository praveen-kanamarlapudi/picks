...
/// `dump/.KeepTrash/` on the same volume (undo is a rename back).
public enum DiskTrash: Sendable {
    public static let keepTrashFolder = ".KeepTrash"
    ...
    public static func usedKeepTrash(_ map: [URL: URL]) -> Bool {
        map.values.contains { $0.pathComponents.contains(keepTrashFolder) }
    }
    ...
    private static func trashOne(_ url: URL, dumpRoot: URL) throws -> URL {
        ...
        return try moveToKeepTrash(url, dumpRoot: dumpRoot)
    }

    public static func moveToKeepTrash(_ url: URL, dumpRoot: URL) throws -> URL {
        let rel = Indexer.relativePath(url, root: dumpRoot)
        let dest = PhotoRecord.canonicalURL(
            root: dumpRoot.appendingPathComponent(keepTrashFolder, isDirectory: true),
