...
final class DiskTrashTests: XCTestCase {
    func testTrashAndRestoreRoundTrip() async throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("keep-trash-\(UUID().uuidString)", isDirectory: true)
    ...
    func testInsertPreservingID() throws {
        let db = try Catalog.open(at: FileManager.default.temporaryDirectory
            .appendingPathComponent("keep-trash-id-\(UUID().uuidString).sqlite"))
    ...
    func testKeepTrashFallbackIsUndoable() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("keep-trash-fb-\(UUID().uuidString)", isDirectory: true)
        ...
        try Data("raw".utf8).write(to: raw)
        let dest = try DiskTrash.moveToKeepTrash(raw, dumpRoot: dir)
        XCTAssertFalse(FileManager.default.fileExists(atPath: raw.path))
        XCTAssertTrue(dest.path.contains(DiskTrash.keepTrashFolder))
