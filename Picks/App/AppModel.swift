...
final class AppModel {
    ...
    func reloadPhotos(restorePk: Int64? = nil) throws {
        guard let db else { return }
        let stayID = restorePk ?? current?.id
        ...
        if let stayID, let i = photos.firstIndex(where: { $0.id == stayID }) {
            index = i
    ...
    private func askForWriteAccess(dump: URL) async -> Bool {
        ...
        panel.message = "Picks needs write access to delete photos. Select this same dump folder."
    ...
    func saveExport() {
        ...
        let dest = url.appendingPathComponent("Picks-shortlist-\(stamp)", isDirectory: true)
        do {
