...
final class ExporterTests: XCTestCase {
    ...
    func testShortlistQuery() throws {
        let db = try Catalog.open(at: FileManager.default.temporaryDirectory
            .appendingPathComponent("picks-export-\(UUID().uuidString).sqlite"))
        try db.write { db in
    ...
    func testCeremonyStatsCounts() throws {
        let db = try Catalog.open(at: FileManager.default.temporaryDirectory
            .appendingPathComponent("picks-stats-\(UUID().uuidString).sqlite"))
        try db.write { db in
    ...
    func testCeremonyStatsSplitsCandidAndTraditional() throws {
        let db = try Catalog.open(at: FileManager.default.temporaryDirectory
            .appendingPathComponent("picks-styles-\(UUID().uuidString).sqlite"))
        try db.write { db in
