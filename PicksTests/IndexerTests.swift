...
final class IndexerTests: XCTestCase {
    ...
    func testCollapseMisindexedFlatDump() throws {
        let dbURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("picks-flat-\(UUID().uuidString).sqlite")
        let db = try Catalog.open(at: dbURL)
        try db.write { db in
            var a = PhotoRecord(
                photoID: "SAB09390",
    ...
    func testCollapseLeavesNestedDumpsAlone() throws {
        let dbURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("picks-nested-\(UUID().uuidString).sqlite")
        let db = try Catalog.open(at: dbURL)
        try db.write { db in
            var a = PhotoRecord(
                photoID: "AKHI0001",
    ...
    private func makeMiniDump() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PicksMini-\(UUID().uuidString)", isDirectory: true)
        let paths = [
